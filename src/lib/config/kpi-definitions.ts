...
export const KPI_DEFINITIONS: Record<string, KpiDefinition> = {
  ...
  parkAndPayUtilization: {
    id: 'parkAndPayUtilization',
    name: 'Park & Pay Utilization',
    description: 'Utilization of pallet positions rented from third parties and sold on to customers. Reported separately from the own network because it is a different commercial book with a different cost base.',
    unit: 'percent',
    formula: 'sum(site.utilizedPallets) / sum(site.capacity) * 100',
    target: THRESHOLDS.networkTargetPct,
    warningThreshold: THRESHOLDS.breachThresholdPct,
    criticalThreshold: 100,
    direction: 'band',
    source: 'Park & Pay daily grid',
    owner: 'Commercial · Network Planning',
    refreshFrequency: 'DAILY_0545',
  },
  combinedUtilization: {
    id: 'combinedUtilization',
    name: 'Combined Utilization (Own + Park & Pay)',
    description: 'Own and Park & Pay capacity and occupancy summed and divided once. Never the average of the two percentages, which would misstate the network whenever the two books differ in size.',
    unit: 'percent',
    formula: '(own.utilizedPallets + pnp.utilizedPallets) / (own.capacity + pnp.capacity) * 100',
    target: THRESHOLDS.networkTargetPct,
    warningThreshold: THRESHOLDS.breachThresholdPct,
    criticalThreshold: 100,
    direction: 'band',
  ...
  parkAndPayCapacity: {
    id: 'parkAndPayCapacity',
    name: 'Park & Pay Capacity',
    description: 'Pallet positions contracted from third-party sites. The source grid publishes a single figure per location with no frozen / chilled / dry split.',
    unit: 'pallets',
    formula: 'sum(site.capacity)',
    target: null,
    warningThreshold: null,
    criticalThreshold: null,
    direction: 'band',
